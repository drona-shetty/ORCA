    ...
    <section class="section">
        <div class="card">
            <div class="card-body">
                ...
                @endif
                <form class="row g-3" enctype="multipart/form-data" method="POST" action="{{url('yn-admin/gcns23/about')}}">
                    @csrf
                    ...
                    <tbody>
                        @foreach ($abouts as $about)
                            <tr>
                                ...
                                <td width="20%"> <img src="{{ URL::asset('images/event/media/'.$about->image) }}" alt="{{ $about->image }}"> </td>
                                <td width="50%"> {{ $about->desc }} </td>
                                <td width="10%">
                                    <a href="{{ url('yn-admin/gcns23/about') }}/{{ $about->id }}/edit" class="btn btn-primary rounded-pill btn-sm">Edit</a>
                                    <button data-bs-toggle="modal" data-id="{{$about->id}}" data-aurl="{{ url('check-category') }}" data-url="{{ url('yn-admin/gcns23/about/'.$about->id) }}"
